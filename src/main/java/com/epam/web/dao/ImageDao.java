...
public class ImageDao<I extends Image> extends AbstractDao<I> {
    ...
    private final String SQL_SELECT_LIMITED_SPECIALIZATION_IMAGES;
    ...
    public ImageDao(ProxyConnection connection, Mapper<I> mapper, String tableName) {
        ...
        this.SQL_SELECT_LIMITED_FACULTY_IMAGES ="SELECT id,path FROM "+tableName+" JOIN faculty ON faculty.faculty_id="+tableName+".id";
        this.SQL_SELECT_LIMITED_SPECIALIZATION_IMAGES ="SELECT id,path FROM "+tableName+" JOIN specialization ON specialization.specialization_id="+tableName+".id AND specialization.faculty_id=?";
